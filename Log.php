...
class Log
{
     ...
     * @return bool
     */
    public function apisave(array $log, $topic = ''): bool
    {
        $info = [];
        // 日志信息封装
        $time = time();
        $logItem = new \Aliyun_Log_Models_LogItem();
        $logItem->setTime($time);
        $runtime = (number_format(microtime(true), 8, '.', '') - App::getBeginTime()) ?: 0.00000001;
        $reqs = number_format(1 / number_format($runtime, 8), 2);
        $log['differenct_time'] = number_format($runtime, 6);
        $log['create_at'] = date('Y-m-d H:i:s');
        $logItem->setContents($log);
        $message[0] = $logItem;
        $info = $message;
        if ($info) {
            return $this->write($info,$topic);
        }
        return true;
    }
    /**
     ...
     * @return bool
     */
    protected function write(array $message, $topic = ''): bool
    {
        empty($topic) && $topic = config('log.aliyun_log_topic');
        ...
        $req = new \Aliyun_Log_Models_PutLogsRequest(config('log.aliyun_log_project'), config('log.aliyun_log_logstore'), $topic, config('log.aliyun_log_source'), $message);
        $client->putLogs($req);
        return true;
    }
}
